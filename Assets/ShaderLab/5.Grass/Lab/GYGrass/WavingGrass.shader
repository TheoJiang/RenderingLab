Shader "Grass_GY"
{
    Properties
    {
        ...
        _TransparentType ("是否在UV的V方向上叠加透明", Range(0, 1)) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            VertexOutput vert(VertexInput v)
            {
                ...
                float3 color = tex2Dlod(_GustNoise, float4(uvW, 0, 0)).xyz;
                float3 color1 = tex2Dlod(_GustNoise, float4(uvW1, 0, 0)).xyz;
                ...
                o.color = float4(combineColor, 1);
                //color = color*color;
                float3 center = float3(0, 0, 0);
                float3 eyeOS = TransformWorldToObject(float4(_WorldSpaceCameraPos.xyz, 1));
            ...
            half4 frag(VertexOutput i): SV_Target
            {
                ...
                //float2 uvW = _WindFrequency * 0.01 * i.positionWS.xz + (-0.01) * i.positionWS.xz * _WindSpeed * _TimeParameters.y * normalize(_WindDirect.xz);
                //half4 gust = tex2D(_GustNoise, uvW);
                half4 col = tex2D(_MainTex, i.uv);
                col.xyz += i.color.xyz * _WindTintStrength;
                //half4 mixColor = lerp(_BottomColor, _TopColor, i.uv.y);
                half4 mixColor = lerp(_BottomColor, _TopColor, pow(i.uv.y, 0.2));
                col *= mixColor;
                //col.a = i.uv.y;
                col.a *= lerp(pow(i.uv.y, 0.99) * col.a, col.a, _TransparentType) ;
                //col*=mixColor;
                // apply fog

Shader "Custom/PPOutline"
{
    Properties
    {
        [HideInInspector]_MainTex("Base (RGB)", 2D) = "white" {}
        _Rate("Rate", Float) = 0.5
        _Strength("_HighLight", Float) = 0.0
...
//            }
            ...
            Varyings vert(Attributes input)
            {
                ...
                output.uv[0] = input.uv + _MaskTexture_TexelSize.xy * half2(-1, -1) * _Rate;
                output.uv[1] = input.uv + _MaskTexture_TexelSize.xy * half2(0, -1) * _Rate;
                output.uv[2] = input.uv + _MaskTexture_TexelSize.xy * half2(1, -1) * _Rate;
                output.uv[3] = input.uv + _MaskTexture_TexelSize.xy * half2(-1, 0) * _Rate;
                output.uv[4] = input.uv + _MaskTexture_TexelSize.xy * half2(0, 0) * _Rate;
                output.uv[5] = input.uv + _MaskTexture_TexelSize.xy * half2(1, 0) * _Rate;
                output.uv[6] = input.uv + _MaskTexture_TexelSize.xy * half2(-1, 1) * _Rate;
                output.uv[7] = input.uv + _MaskTexture_TexelSize.xy * half2(0, 1) * _Rate;
                output.uv[8] = input.uv + _MaskTexture_TexelSize.xy * half2(1, 1) * _Rate;
                return output;
            }
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                // 值越大， 越是边
                // 越小，越是原色
                float4 ol = lerp(_Strength * float4(1,1,1,1), _OutlineColor,  edge);
                return lerp(color , ol, edge);
            }
